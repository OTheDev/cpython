...
#define BITWISE_OP_SIZE_GTE(binop, z, a, b, size_z, size_a, size_b, negz, nega, negb) do { \
    Py_ssize_t i; \
    digit carry_z = negz; \
    if (!nega && !negb) { \
        /* a >= 0, b >= 0 ==> z >= 0. \
        ...
        if (!negz) { \
            ...
            case '^': \
                for (; i < size_z; ++i) { \
                    carry_z += a->long_value.ob_digit[i]; \
                    z->long_value.ob_digit[i] = carry_z & PyLong_MASK; \
                    carry_z >>= PyLong_SHIFT; \
                } \
                break; \
        ...
        if (!negz) { \
            ...
            case '^': \
                for (; i < size_z; ++i) { \
                    carry_a += a->long_value.ob_digit[i] ^ PyLong_MASK; \
                    carry_z += (carry_a ^ PyLong_MASK) & PyLong_MASK; \
                    z->long_value.ob_digit[i] = carry_z & PyLong_MASK; \
                    carry_a >>= PyLong_SHIFT; \
                    carry_z >>= PyLong_SHIFT; \
                } \
        ...
        if (!negz) { \
            ...
            case '&': \
                for (; i < size_z; ++i) { \
                    carry_a += a->long_value.ob_digit[i] ^ PyLong_MASK; \
                    z->long_value.ob_digit[i] = carry_a & PyLong_MASK; \
                    carry_a >>= PyLong_SHIFT; \
                } \
                break; \
            case '^': \
            ...
            case '&': \
                for (i = 0; i < size_b; ++i) { \
                    carry_a += a->long_value.ob_digit[i] ^ PyLong_MASK; \
                    carry_b += b->long_value.ob_digit[i] ^ PyLong_MASK; \
                    carry_z += ~(carry_a & carry_b) & PyLong_MASK; \
                    z->long_value.ob_digit[i] = carry_z & PyLong_MASK; \
                    carry_a >>= PyLong_SHIFT; \
                    carry_b >>= PyLong_SHIFT; \
                    carry_z >>= PyLong_SHIFT; \
    ...
    if (negz) { \
        z->long_value.ob_digit[size_z] = carry_z; \
        _PyLong_FlipSign(z); \
    } \
} while(0)
